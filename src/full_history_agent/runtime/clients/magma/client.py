...
class MagmaCommander(CausalModelClient):

    def __init__(self, settings: ModelSettings, name: str = "commander") -> None:
        super().__init__(settings, name)
        self.output_style = settings.output_style
        if not self.tokenizer.chat_template:
            raise ValueError("A trained commander must supply its chat template")
    ...
    def _format_batch(self, message: BatchedMessageCommander) -> List[str]:
        ...
        for index in range(batch_size):
            ...
            self.input_elements.append({"messages": messages, "tools": message.function[index],
                                        "permanent_rules": get_memory_list(memory),
                                        "summary": summary, "task_attributes": message.attributes[index]})
