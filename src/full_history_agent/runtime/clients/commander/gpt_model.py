...
class OSSCommander(CausalModelClient):
    ...
    def _format_tokens(self, message: BatchedMessageCommander) -> List[List[int]]:
        conversations = []
        self.input_elements = []
        prefill_ids = []
        ...
        for i in range(len(message.instruction)):
            ...
            self.input_elements.append({"history": message.history[i], "instruction": message.instruction[i],
                                        "instruction_role": instruction_roles[i], "attributes": message.attributes[i],
                                        "memory_list": get_memory_list(message.memory[i]), "tools": tools})
    ...
    def _format_batch(self, message: BatchedMessageCommander) -> List[str]:
        return [self.encoding.decode(tokens) for tokens in self._format_tokens(message)]

    def process_batched_entry(self, message: BatchedMessageCommander, inference_mode: bool) -> List[Dict]:
        prefill_ids = self._format_tokens(message)
    ...
    def _ensure_harmony_available() -> None:
        if load_harmony_encoding is None:
            raise ModuleNotFoundError(
                ...
                "Install the agent dependencies or run 'pip install openai-harmony'."
            )
